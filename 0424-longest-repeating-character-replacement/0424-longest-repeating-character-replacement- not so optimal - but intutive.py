class Solution(object):
    def characterReplacement(self, s, k):
        ...
        At each point, we see the frequency of the current character, increment the character count, 
        WE WILL UPDATE the max frequency by calculating the maximum of the frequency in the frequency dictionary. This will add the Time Complexity to O(N) per each iteration.
        ...
        for current in range(0,len(s)):
            ...
            #adds the time complexity of O(N) in each case
            freq_max_repeated_char = max(freq_dict.values())
